...
class Diffusion(L.LightningModule):
  ...
  def sample(
    ...
    # Adi: this is where we get CBG
    if getattr(self.config, 'guidance', None) is not None: # if there are guidance settings
      if self.config.guidance.method == 'cfg': # classifier-free guidance
        cond = (torch.ones(self.config.sampling.batch_size, device=self.device) *
                self.config.guidance.condition).to(torch.long)
      else:
        cond = None # if classifier-based guidance, cond is None = there's no conditioning
          # CFG is training a unconditional model and a conditional model, and the "trick" is to
          # use the unconditional model as a "negative" sample for the conditional model.
          # and the conditional model is set by using cond_dropout in the training config.
      if ((self.parameterization == 'ar' and self.config.guidance.method in {'fudge', 'pplm'})
          or self.config.guidance.method in {'cbg', 'nos'}):
        # Adi: there are several options for classifier-based guidance, one of them is CBG if the parameterization
        # is not AR.
        classifier_model = classifier.Classifier.load_from_checkpoint(
          # Adi: this is the heart of the classifier-based guidance: where we load the classifier model
          # the classifier model is imported from the classifier.py and the Classifier class
          # it returns a logits tensor from the forward and a softmax log prob of the logits from a special
          # function.
          self.config.guidance.classifier_checkpoint_path,
          tokenizer=self.tokenizer,
          config=self.config, logger=False).to(self.device)
          # Adi: interesting to see that we pass a tokenizer and a config to the classifier model
          # but this is just to give the vocab size and the special tokens ids
        classifier_model.eval() # we're not training the classifier model, so we set it to eval mode
      else: # else, there's no CBG
        classifier_model = None
    else: # else there's no guidance
