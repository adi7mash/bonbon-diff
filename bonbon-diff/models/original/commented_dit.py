...
class LabelEmbedder(nn.Module):
  ...
  def __init__(self, num_classes, cond_size):
    super().__init__()
    # Adi: this is used for getting the conditioning embeddings
    # but here it is only for class labels, not embeddings of another modality
    # so we would need to modify this code, but basically we would need to project
    # instead of using the embedding table nn.Embedding
